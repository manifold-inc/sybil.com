...
interface CompletionCodeExampleProps {
  model: string | null;
}
...
export const CompletionCodeExample = ({
  model,
}: CompletionCodeExampleProps) => {
  const auth = useAuth();
  ...
  const getCodeExample = (lang: typeof selectedLang) => {
    const examples = {
      curl: `curl ${env.NEXT_PUBLIC_CHAT_API}/v1/completions \\
...
void chat();`,
  ...
  if (!model) {
    return (
      <div className="no-scrollbar h-80 overflow-y-auto">
      ...
      <div className="bg-mf-card-dark rounded-md h-full w-full flex flex-col border border-mf-metal-300">
        <div className="flex items-center justify-between px-4 py-2.5">
          ...
          <div className="flex items-center gap-1.5">
            <p className="font-saira">Completion</p>
            <button
          ...
          <Menu as="div" className="relative inline-block sm:hidden">
            ...
            <Menu.Items className="absolute z-10 w-32 -translate-x-24 mt-1 rounded-md border outline-none bg-mf-card-dark border-mf-ash-300">
              {languages.map((lang) => (
        ...
        {/* Code Example */}
        <div className="no-scrollbar overflow-y-auto text-xs">
          <SyntaxHighlighter
